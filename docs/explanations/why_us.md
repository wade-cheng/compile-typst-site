...
Compiling by hand doesn't natively include a local auto-reloading web server. When your browser opens a plain HTML file, it might not load your CSS if it's linked in a separate file. This is a security feature in browsers, iiuc. This means you have to, e.g., `python -m http.server`, or only view your site after uploading it to Github Pages or Neocities.

Every solution but manual compilation spins up a web server locally (to, for example, <http://localhost:8000/>) and even reloads the page when save your files, automatically.
